...
# ================= CONFIG =================
BOT_TOKEN = "8733154492:AAFW0VGswCOf-nhsf6FlCfU8i1KSaEfGYxA"  # SHU YERGA TOKEN QO'Y
ADMIN_ID = 7253593181
DB_FILE = "db.json"
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uc, price in PACKAGES.items():
        keyboard.append([
            InlineKeyboardButton(f"{uc} UC - {price} so'm", callback_data=uc)
        ])
...
async def addbal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=int(uid),
            text=f"💰 Sizga +{amount} so'm berildi"
        )
