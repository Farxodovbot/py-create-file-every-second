...
async def admin_panel(message: types.Message):
    ...
    kb.add("➕ Anime qo'shish", "📢 Kanalga xabar", "🗑 O'chirish")
...
async def post_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 Ma'lumotlarni yuboring (Nomi, Tili, Yili, Janri...):")

@dp.message_handler(state=ChannelPost.info)
async def post_info(message: types.Message, state: FSMContext):
    await state.update_data(info=message.text)
    await ChannelPost.next()
    await message.answer("🔢 Ushbu anime kodini yuboring:")
...
async def post_finish(message: types.Message, state: FSMContext):
    ...
    btn = InlineKeyboardMarkup().add(InlineKeyboardButton("📺 Tomosha qilish", url=f"https://t.me/{bot_info.username}?start={code}"))
    await bot.send_photo(CHANNEL_ID, data['photo'], caption=f"{data['info']}\n\n🔢 <b>Kod:</b> <code>{code}</code>", reply_markup=btn)
    await state.finish()
...
# --- O'CHIRISH ---
@dp.message_handler(text="🗑 O'chirish", user_id=ADMIN_ID)
async def del_start(message: types.Message):
    await message.answer("O'chirmoqchi bo'lgan anime kodini yuboring:\n(Masalan: /del 733)")

@dp.message_handler(commands=['del'], user_id=ADMIN_ID)
async def delete_anime(message: types.Message):
    code = message.get_args()
    cur.execute("DELETE FROM animelar WHERE code=?", (code,))
    cur.execute("DELETE FROM qismlar WHERE code=?", (code,))
    conn.commit()
...
async def send_anime(message, code):
    ...
    else: await message.answer("❌ Kechirasiz, bu kod bilan anime topilmadi.")
...
async def recheck(c: types.CallbackQuery):
    ...
    if m.status in ['member', 'creator', 'administrator']:
        await c.message.delete(); await c.message.answer("✅ Rahmat! Endi foydalanishingiz mumkin.")
    else: await c.answer("❌ A'zo emassiz!", show_alert=True)

# --- RENDER UCHUN ASYNC START ---
