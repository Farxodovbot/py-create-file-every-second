...
# 2. SOZLAMALAR (Yangi token bilan)
...
def save_anime(message, p_id):
    ...
    except: bot.send_message(message.chat.id, "❌ Xato! Format: `Nomi | Kod | Tavsif` (Ajratuvchi chiziqqa e'tibor bering)")

@bot.message_handler(func=lambda m: m.text == "🎬 Qism qo'shish")
def admin_ep_start(message):
    if message.from_user.id == ADMIN_ID:
...
def msg_handler(message):
    # Admin panel buyruqlari bo'lsa, kod deb tekshirmaslik:
    if message.from_user.id == ADMIN_ID and (message.text == "/admin" or message.text in ["➕ Anime qo'shish", "🎬 Qism qo'shish"]):
        return
