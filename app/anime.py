...
def start_cmd(message):
    ...
    if len(args) > 1:
        ...
        bot.send_message(message.chat.id, "👋 Salom! Kodni yuboring.")
...
def send_anime_info(chat_id, code):
    ...
    markup.add(*btns)
    bot.send_photo(chat_id, anime['rasm'], caption=f"📺 **{anime['nomi']}**", reply_markup=markup, parse_mode="Markdown")
...
def play_video(call):
    ...
    bot.send_video(call.message.chat.id, v_id)
