...
cur = conn.cursor()
cur.execute('''CREATE TABLE IF NOT EXISTS animelar 
               (code TEXT PRIMARY KEY, title TEXT, photo TEXT, eps_count INTEGER)''')
cur.execute('''CREATE TABLE IF NOT EXISTS qismlar 
               (code TEXT, ep_num INTEGER, link TEXT)''')
conn.commit()
...
async def post_finish(message: types.Message, state: FSMContext):
    ...
    btn = InlineKeyboardMarkup().add(
        InlineKeyboardButton("📺 Tomosha qilish", url=f"https://t.me/{bot_info.username}?start={code}")
    )
    
    caption = f"{data['info']}\n\n🔢 <b>Kod:</b> <code>{code}</code>"
    await bot.send_photo(CHANNEL_ID, data['photo'], caption=caption, reply_markup=btn)
    await state.finish()
...
async def load_title(message: types.Message, state: FSMContext):
    ...
    await AnimeAdd.next()
    await message.answer("🎞 Qismlar sonini yozing:")
...
async def load_eps(message: types.Message, state: FSMContext):
    ...
    await AnimeAdd.next()
    await message.answer("🔢 Anime kodini bering:")
...
async def load_code(message: types.Message, state: FSMContext):
    ...
    await AnimeAdd.next()
    await message.answer("🔗 1-qism linkini yuboring:")
...
async def load_links(message: types.Message, state: FSMContext):
    ...
    if data['current_ep'] < data['eps_count']:
        ...
        conn.commit()
        await state.finish()
        await message.answer("✅ Anime bazaga qo'shildi!")
