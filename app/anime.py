...
# --- 🔵 ADMIN PANEL (SIZ AYTGAN TUGMALAR BILAN) ---
...
def save_anime(message, p_id):
    ...
    except: bot.send_message(message.chat.id, "❌ Xato! Tayoqcha `|` dan foydalaning.")
...
def get_code_for_ep(message):
    ...
    else: bot.send_message(message.chat.id, "❌ Bunday kodli anime yo'q!")
...
def finalize_delete(message):
    ...
    if k in anime_db:
        del anime_db[k]
        bot.send_message(message.chat.id, f"✅ Anime (Kod: {k}) bazadan o'chirildi!")
    else:
...
def msg_handler(message):
    ...
    # Admin xabarlarini kod deb o'ylamaslik uchun
    if int(message.from_user.id) == ADMIN_ID:
        if message.text == "/admin" or message.text in ["➕ Anime qo'shish", "🎬 Qism qo'shish", "🗑 Animeni o'chirish"]:
            return
