...
# 2. SOZLAMALAR
TOKEN = 8635058894:AAE318FprIsxbJpmuSGsZe9LCkT1VeE0zR4
ADMIN_ID = 7253593181
...
# --- 🔵 ADMIN PANEL (SIZ AYTGANDREK TUGMALAR) ---
...
def save_anime(message, p_id):
    try:
        ...
        bot.send_message(message.chat.id, f"✅ Anime bazaga qo'shildi! Kod: `{k}`")
    except: bot.send_message(message.chat.id, "❌ Xato! Format: `Nomi | Kod | Tavsif`")

# 2. QISM QO'SHISH
@bot.message_handler(func=lambda m: m.text == "🎬 Qism qo'shish")
def add_ep_start(message):
    if int(message.from_user.id) == ADMIN_ID:
        msg = bot.send_message(message.chat.id, "🔢 Qaysi kodli animega qism qo'shmoqchisiz?")
        bot.register_next_step_handler(msg, get_code_for_ep)

def get_code_for_ep(message):
    k = message.text
    if k in anime_db:
        msg = bot.send_message(message.chat.id, f"🎞 {anime_db[k]['nomi']} uchun video (qism) yuboring:")
        bot.register_next_step_handler(msg, save_video, k)
    else: bot.send_message(message.chat.id, "❌ Bunday kodli anime bazada yo'q!")

def save_video(message, k):
    if message.video:
        q_soni = len(anime_db[k]['qismlar']) + 1
        anime_db[k]['qismlar'][str(q_soni)] = message.video.file_id
        bot.send_message(message.chat.id, f"✅ {q_soni}-qism muvaffaqiyatli saqlandi!")
    else: bot.send_message(message.chat.id, "❌ Bu video emas!")
...
def finalize_delete(message):
    k = message.text
    if k in anime_db:
        nomi = anime_db[k]['nomi']
        del anime_db[k]
        bot.send_message(message.chat.id, f"✅ '{nomi}' bazadan butunlay o'chirildi!")
    else:
...
def msg_handler(message):
    ...
    if int(message.from_user.id) == ADMIN_ID:
        if message.text in ["/admin", "➕ Anime qo'shish", "🎬 Qism qo'shish", "🗑 Animeni o'chirish"]:
            return
